...
public class LotteryGlobalMenu extends Menu {
    ...
    public String getTitle(Player player) {
        return CC.translate(adminMenus.getString("LOTTERY.GLOBAL.TITLE"));
    }

    @Override
    public int getSize() {
        return adminMenus.getInt("LOTTERY.GLOBAL.SIZE");
    }

    @Override
    public Map<Integer, Button> getButtons(Player player) {
        Map<Integer, Button> buttons = new HashMap<>();
        buttons.put(adminMenus.getInt("LOTTERY.GLOBAL.BUTTONS.LIST.SLOT"), new LotteryListButton());
        buttons.put(8, new BackButton(new CelestEditorMenu()));
        setPlaceholder(adminMenus.getBoolean("LOTTERY.GLOBAL.FILLER"));
        return buttons;
    }
}
