...
public class LotteryPaginatedMenu extends PaginatedMenu {
    ...
    public String getPrePaginatedTitle(Player player) {
        return CC.translate(adminMenus.getString("LOTTERY.LIST.TITLE"));
    }
    ...
    public Map<Integer, Button> getAllPagesButtons(Player player) {
        ...
        for (Lottery lottery : ModuleService.getManagerModule().getLotteryManager().getLotteries()) {
            buttons.put(index++, new LotteryPaginatedButton(lottery.getName()));
        }
    ...
    public Map<Integer, Button> getGlobalButtons(Player player) {
        ...
        buttons.put(getSlot(3, getSize() / 9 - 1), new LotteryReminderIntervalButton());
        buttons.put(getSlot(4, getSize() / 9 - 1), new LotteryCreateButton());
