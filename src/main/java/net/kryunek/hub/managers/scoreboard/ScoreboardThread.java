...
public class ScoreboardThread {
    ...
    private void tick() {
        for (Player player : this.assemble.getPlugin().getServer().getOnlinePlayers()) {
            try {
                ScoreboardBoard board = this.assemble.getBoards().get(player.getUniqueId());
                ...
                if (newLines == null || newLines.isEmpty()) {
                    ...
                    for (int i = 0; i < newLines.size(); i++) {
                        ...
                        // Creating a new ScoreboardBoardEntry instance will add
                        // itself to the provided board's entries list.
                        if (entry == null) {
                            entry = new ScoreboardBoardEntry(board, line, i);
                        }
